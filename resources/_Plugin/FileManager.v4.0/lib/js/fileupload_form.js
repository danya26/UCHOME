...
$.extend(FormUploader.prototype, {
	...
	, _CreateUploadStyle : function(target, inst){	// 기본 폼 생성
		...
		upload_form.setAttribute("id","frm_" + inst.settings.prg_sub_code);	// smartcreator 때문에 추가
		...
		$(target).append(mfu_top);
			...
			$(mfu_fileselect_btn).attr('id', 'btn_' + inst.settings.prg_sub_code);	// smartcreator 때문에 추가
			$(mfu_fileselect_btn).html("파일 선택");
		...
		$.ajax({
			...
			success : function(response){
				...
				if(inst.settings.isHelpInfo){
					...
					$(mfu_helpinfo_div).append( "<ul class='c-list01'></ul>" );

					$("ul.c-list01", $(mfu_helpinfo_div) ).append("<li>총 '" + file_config_info.prg_file_tot_upl_cnt + "'개의 파일을 첨부할수 있습니다.</li>");
					$("ul.c-list01", $(mfu_helpinfo_div) ).append("<li>파일당 용량 제한은 '" + file_config_info.prg_file_upl_size + "MB' 입니다.</li>");
					$("ul.c-list01", $(mfu_helpinfo_div) ).append("<li>업로드는 [" + file_config_info.prg_file_app_exe.split(";").join(", ")+ "]파일만 가능합니다.</li>");
					if(file_config_info.prg_img_limit_width != 0 && file_config_info.prg_img_limit_height != 0){
						$("ul.c-list01", $(mfu_helpinfo_div) ).append("<li>이미지 파일의 경우 가로 세로 '" + file_config_info.prg_img_limit_width + "px Ⅹ " + file_config_info.prg_img_limit_height + "px 이하'만 업로드 가능합니다.</li>");
					}
					$("ul.c-list01", $(mfu_helpinfo_div) ).append("<li>업로드 파일 제한 해제는 관리자에게 문의 해주세요.</li>");
					var bt_del = document.createElement("button");
					$(mfu_bt_help).bind("click", function(){
						$(mfu_helpinfo_div).toggle();
					});
				}
			},
			...
			error : function(xhr, status, error) { //alert("에러발생:" + error + status + xhr);
				...
				alert("파일 업로드 모듈 로드중 에러가 발생 했습니다.\n관리자에게 문의주세요.");
			}
		});
	}
	...
	, _FileItemStateChange : function(inst, item, state, li_itm){
		...
		if(state == "Y"){
			...
			$(data_target.file_img).prop("alt", item.file_exe + "파일 아이콘 이미지");

			$(data_target.file_stat).html("등록일 : " + item.reg_date.substr(0, 16));
			...
			if(inst.settings.isFileView){
				$(data_target.obj_bt_view).unbind("click");
				$(data_target.obj_bt_view).bind("click", function(){ $formuploader._OnClick_ImageViewBtn(inst, item.download_url, inst.settings.prg_code, item.idx)});
			...
			$(data_target.file_img).prop("alt", "파일업로드에 실패 했습니다.");
			$(data_target.file_size).html("실패");
			...
			$(data_target.file_img).prop("alt", "파일 업로드 중");
			$(data_target.file_size).html(file_size(0));
			$(data_target.file_stat).html("업로드중...");
	...
	, _AddFileItem : function(inst, item, state){	// 기존 파일 등록 또는 업로드 시 사용
		...
		$(file_item).data("idx", item.idx);
			...
			$(file_item).append(mfu_face);
				...
				$(bt_menu_open).html("<span>메뉴열기</span>");
				...
				FILE_OBJ.file_ctrls = mfu_ctrls;
					...
					$(bt_menu_close).html("<span>메뉴닫기</span>");
				...
				FILE_OBJ.file_fcont = mfu_fcont;
					...
					FILE_OBJ.file_fcont_content = mfu_fcont_content;
						...
						$(mfu_fcont_content).append(isFileAlt);
							...
							$(bt_add_cont).html("<span>본문추가</span>");
							$(bt_add_cont).hide();
					...
					$(bt_cont_close).html("<span>취소</span>");
	...
	, _OnClick_FileSelector : function(inst){
		//파일 미 선택 시 리턴
		if(!$(inst.settings.upload_form).find("input[name=file_item]").val()){
			return;
		}
		...
		$(inst.settings.upload_form).ajaxForm({
	        beforeSubmit: function (data, form, option) {
	        	...
	        	if((inst.settings.upload_sucess_file_cnt + file_cnt)<= inst.settings.file_config_info.prg_file_tot_upl_cnt){
	        		...
	        		alert("파일을 추가 할 수 없습니다.\n추가 가능한 파일은 '" + (inst.settings.file_config_info.prg_file_tot_upl_cnt - inst.settings.upload_sucess_file_cnt)+ "'개 입니다.");
	        		return false;
	        	}
	        },
	...
	, _OnClick_ImageViewBtn : function(inst, qcode, prg_code, idx) {	// 파일 다운로드 기능

		var option = {
			url: "/"+inst.settings.site_code+"/ajx_json/UploadMgr/ImageView.do",
			parameter: {
				qcode: qcode
				, prg_code: prg_code
				, idx: idx
			},
			name: "imageView_popup",
			width: "600",
			height: "600",
			scrollbars: "yes",
			resizable: "yes"
			...
			if($(item.li).data("idx") == 0){
				...
				$.ajax({
					...
					error : function(xhr, status, error) {//alert("에러발생:" + error + status + xhr);
						...
						alert("삭제가 실패하였습니다. 관리자에게 문의 주세요.");
					}
				});
			}
		}
	...
	, _OnClick_DownloadBtn : function(inst, qcode, prg_code){	// 파일 다운로드 기능
		...
		$.ajax({
			...
			error : function(xhr, status, error) {//alert("에러발생:" + error + status + xhr);
				...
				alert("다운로드가 실패하였습니다. 관리자에게 문의 주세요.");
			}
		});
	}
