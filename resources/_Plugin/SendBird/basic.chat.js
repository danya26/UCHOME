	var appId = '5AD58897-41FC-4EAF-95C6-7FBA78AD4F2D';
	...
	function getMemberList(channel) {
		if ( channel.isOpenChannel() ) {
			...
			members.forEach(function (member) {
				...
				'  <div class="modal-member-list__name">' + (member.nickname.length > 13 ? xssEscape(member.nickname.substring(0, 12)) + '...' : xssEscape(member.nickname)) +
				'  </div>' +
				'</div>';
			});
		...
		if ( channel.isGroupChannel() ) {
			...
			memberListHtml += '' +
				...
				'  <div class="modal-member-list__name ' + isOnline + '">' + (member.nickname.length > 13 ? xssEscape(member.nickname.substring(0, 12)) + '...' : xssEscape(member.nickname)) +
				'  </div>' +
	...
	function updateGroupChannelLastMessage(message) {
		...
		if ( message ) {
			...
			if (lastMessageDateString) {
				lastMessageDateString = '<div><img src="/resources/_Plugin/SendBird/img/icon-time.png" style="vertical-align:moddle;padding-bottom:2px;"> <span> ' + lastMessageDateString + '</span></div>';
			}
		...
		if ( message.isAdminMessage() ) {
			...
			if ( isCurrentUser(user.userId) ) {
				...
				var msg = '' +
					...
					'  <label class="chat-canvas__list-name chat-canvas__list-name__user" data-userid="%userid%">' + xssEscape(user.nickname) +
					'  </label>' +
					'  <label class="chat-canvas__list-separator">:</label>' +
					'  <label class="chat-canvas__list-text" data-messageid="%messageid%">%message%</label>' + readReceiptHtml +
					'</div>';
				...
				var msg = '' +
					'<div class="chat-canvas__list">' +
					'  <label class="chat-canvas__list-name" data-userid="%userid%" data-nickname="%nickname%">' + xssEscape(user.nickname) +
					'  </label>' +
					'  <label class="chat-canvas__list-separator">:</label>' +
					'  <label class="chat-canvas__list-text" data-messageid="%messageid%">' + convertLinkMessage(xssEscape(message.message)) +
					'  </label>' +
					'</div>';
	...
	function fileMessageList(message) {
		...
		if ( isCurrentUser(user.userId) ) {
			msgList += '' +
			'<div class="chat-canvas__list">' +
			'  <label class="chat-canvas__list-name chat-canvas__list-name__user">' + xssEscape(user.nickname) +
			'  </label>' +
			'  <label class="chat-canvas__list-separator">:</label>' +
			...
			msgList += '' +
			'<div class="chat-canvas__list">' +
			'  <label class="chat-canvas__list-name" data-userid="%userid%" data-nickname="%nickname%">'.replace('%userid%', user.userId).replace('%nickname%', xssEscape(user.nickname)) + xssEscape(user.nickname) +
			'  </label>' +
			'  <label class="chat-canvas__list-separator">:</label>' +
			'  <label class="chat-canvas__list-text" data-messageid="%messageid%">'.replace('%messageid%', message.messageId) +
			'    <label class="chat-canvas__list-text-file">FILE</label>' +
	...
	function imageMessageList(obj) {
		...
		if ( isCurrentUser(user.userId) ) {
			msgList += '' +
			'<div class="chat-canvas__list">' +
			'  <label class="chat-canvas__list-name chat-canvas__list-name__user">' + xssEscape(user.nickanme) +
			'  </label>' +
			'  <label class="chat-canvas__list-separator">:</label>' +
			'  <label class="chat-canvas__list-text" data-messageid="%messageid%">'.replace('%messageid%', message.messageId) + xssEscape(message.name) +
			'  </label>' +
			...
			msgList += '' +
			'<div class="chat-canvas__list">' +
			'  <label class="chat-canvas__list-name" data-userid="%userid%" data-nickname="%nickname%">'.replace('%userid%', user.userId).replace('%nickname%', xssEscape(user.nickname)) + xssEscape(user.nickanme) +
			'  </label>' +
			'  <label class="chat-canvas__list-separator">:</label>' +
			'  <label class="chat-canvas__list-text" data-messageid="%messageid%">'.replace('%messageid%', message.messageId) + xssEscape(message.name) +
			'  </label>' +
	...
	function setSysMessage(obj) {
		$('.chat-canvas').append(
			'<div class="chat-canvas__list-notice">' +
			'  <label class="chat-canvas__list-system">' + xssEscape(obj['message']) +
			'  </label>' +
			'</div>'
		);
		scrollPositionBottom();
	...
	function setBroadcastMessage(obj) {
		$('.chat-canvas').append(
			'<div class="chat-canvas__list">' +
			'  <label class="chat-canvas__list-broadcast">' + xssEscape(obj['message']) +
			'  </label>' +
			'</div>'
		);
		scrollPositionBottom();
	...
	function addUnreadCount(item, unread, targetUrl) {
		...
		if ( $(item).find('div[class="left-nav-channel__unread"]').length != 0 ) {
			...
			$('<div class="left-nav-channel__unread">' + unread + '</div>').prependTo('.left-nav-channel-group[data-channel-url=' + targetUrl + ']');
		}
	}
