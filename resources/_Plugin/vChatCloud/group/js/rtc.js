...
function mic_on_off(item) {
	if (channel) {
		...
		if (chk == 'mic btn_on') {
			...
			$(img).attr('src', '/resources/_Plugin/vChatCloud/group/img/webRTC/off_mic.png');
			$(cam_mic).show();
			res.toastPopup("마이크 꺼짐.");
		} else {
			$(item).attr('class', 'mic btn_on');
			$(img).attr('src', '/resources/_Plugin/vChatCloud/group/img/webRTC/on_mic.png');
			$(cam_mic).hide();
...
function cam_on_off(item) {
	if (channel) {
		...
		if (chk == 'cam btn_on') {
			...
			$(img).attr('src', '/resources/_Plugin/vChatCloud/group/img/webRTC/off_cam.png');
			res.toastPopup("카메라 꺼짐.");
		} else {
			$(item).attr('class', 'cam btn_on');
			$(img).attr('src', '/resources/_Plugin/vChatCloud/group/img/webRTC/on_cam.png');
...
function videoInit() {
	...
	channel.on('rtcLocalAudioChanged', function(event) {
		...
		// $('.cam-footer .cam-btn .mic img').attr('src',is_mic?'/resources/_Plugin/vChatCloud/group/img/webRTC/on_mic.png':'/resources/_Plugin/vChatCloud/group/img/webRTC/off_mic.png');
	});
	// 채널에 local video 변경시
	channel.on('rtcLocalVideoChanged', function(event) {
		...
		// $('.cam-footer .cam-btn .cam img').attr('src',is_cam?'/resources/_Plugin/vChatCloud/group/img/webRTC/on_cam.png':'/resources/_Plugin/vChatCloud/group/img/webRTC/off_cam.png');
	});
...
class resources {
	...
	get myVideo() {
		return '<!-- 내 비디오 --><div class="camvideo-wrap" name="my_cam"><div class="camvideo"><video autoplay mute style="position:absolute;left:0;top:0;width:100%;height:100%;"></video><img src="/resources/_Plugin/vChatCloud/group/img/webRTC/user.png" class="nocam" alt="영상없음"></div><img src="/resources/_Plugin/vChatCloud/group/img/webRTC/nosound.png" class="nomic" alt="소리없음"><div class="cam-name"><p>나</p></div></div>';
	}
	get remoteVideo() {
		return '<div class="camvideo-wrap"><div class="camvideo"><video autoplay style="position:absolute;left:0;top:0;width:100%;height:100%;"></video><img src="/resources/_Plugin/vChatCloud/group/img/webRTC/user.png" class="nocam" alt="영상없음"></div><img src="/resources/_Plugin/vChatCloud/group/img/webRTC/nosound.png" class="nomic" alt="소리없음"><div class="cam-name"><p>사용자 이름 노출 영역입니다.</p></div></div>';
	}
	toastPopup(msg) {
