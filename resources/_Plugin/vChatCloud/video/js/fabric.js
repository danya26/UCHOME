...
class FabricWrapper {
  constructor(option = { color: 'white' }) {
    ...
    this.setHistory();
    this.changePresenter(this.presenter);
    this.resize();
  ...
  setColorPicker() {
    ...
    $('.shapes_tool_wrap .color_fill').css({
      'background-image': `linear-gradient(${this.shapeColor}, ${this.shapeColor}), url(/resources/_Plugin/vChatCloud/video/img/fabric/transparents.svg)`,
    });
    ...
    $('.pen_tool_wrap .color_fill').css({
      'background-image': `linear-gradient(${this.brush.color}, ${this.brush.color}), url(/resources/_Plugin/vChatCloud/video/img/fabric/transparents.svg)`,
    });
    ...
    this.shapeColorPicker.on('change', (color) => {
      ...
      $('.shapes_tool_wrap .color_fill', this.canvas_tool).css({
        'background-image': `linear-gradient(${newColor}, ${newColor}), url(/resources/_Plugin/vChatCloud/video/img/fabric/transparents.svg)`,
      });
  ...
  setBrushWidth() {
    ...
    this.brush.width = 1;
    // color
    $('.pen_tool_wrap .subtool-colorpicker', this.canvas_tool).on('click', (e) => {
    ...
    // size buttons
    $('li.stroke', this.canvas_tool).on('click', (e) => {
  ...
  setShapeButtons() {
    $('.shapes_tool_wrap .subtool-colorpicker', this.canvas_tool).on('click', (e) => {
      if (
    ...
    $('.shape-circle', this.canvas_tool).on('click', () => {
    ...
    $('.shape-rect', this.canvas_tool).on('click', () => {
      const rect = new Rect({
        top: 100,
        left: 100,
        width: 100,
    ...
    $('.shape-triangle', this.canvas_tool).on('click', () => {
  ...
  setFunction() {
    ...
    $('.subtool_wrap', this.canvas_tool).on('mousemove', () => {
    ...
    // canvas scroll event
    // this.canvas_wrap.on('scroll', (e) => {
    //   if (this.presenter) {
    //     const scrollTop = this.canvas_wrap.scrollTop();
    //     const scrollLeft = this.canvas_wrap.scrollLeft();
  ...
  setButtons() {
          ...
          switch (typeof flag) {
            ...
            case 'string':
              if (flag === 'open') {
                ...
                $('.navi03 .canvas').attr('class', 'canvas on');
              } else if (flag === 'close') {
                this.fabric_wrap.hide();
                $('.navi03 .canvas').attr('class', 'canvas off');
              }
              break;
            // i click btn
            case 'object':
              if (this.fabric_wrap.is(':visible')) {
                this.fabric_wrap.hide();
                $('.navi03 .canvas').attr('class', 'canvas off');
                this.sendMessage('close');
              } else {
                if (this.nowPresent) $('li.Present.on').click();
                this.fabric_wrap.show();
                $('.navi03 .canvas').attr('class', 'canvas on');
    ...
    // canvas버튼 클릭 시 실행
    $('.navi03 .canvas').on('click', this.buttons.canvasBtn);

    // 커서 보이기(레이저 포인터) 버튼
    $('.pointer', this.canvas_tool).on('click', this.buttons.pointerBtn);

    // 도형 메뉴 켜기 버튼
    $('.shapes', this.canvas_tool).on('click', this.buttons.shapesBtn);

    // 펜 버튼
    $('.pen', this.canvas_tool).on('click', this.buttons.penBtn);

    $('.canvas_tool_btn.pen', this.canvas_tool).on('mouseenter', this.buttons.penEvent);

    // 지우개 버튼
    $('.eraser', this.canvas_tool).on('click', this.buttons.eraserBtn);

    // 모두 지우기 버튼
    $('.eraser-all', this.canvas_tool).on('click', this.buttons.eraseAllBtn);

    // 닫기 버튼
    $('.tool_close', this.canvas_tool).on('click', this.buttons.canvasBtn);
  }
  ...
  canvasEvent(data) {
    ...
    switch (event) {
      ...
      //   break;
      case 'pointer_show':
        let pointer = $(`#pointer_${clientKey}`);
        if (pointer.length === 0) {
          pointer = $(this.newPointer);
          pointer.attr('id', `pointer_${clientKey}`);
      ...
      case 'pointer_hide':
        ...
        $(`#pointer_${clientKey}`).hide();
        break;
      // case 'pointer_change':
      //   this.pointerImg.css({ cursor: cursor });
      //   break;
      case 'pointer_move':
        $(`#pointer_${clientKey}`).css(position);
        break;
    }
  }
  ...
  changePresenter(isMe = this.presenter) {
    // if (isMe) {
    //   // presenter
    //   // canvas & objects change options (select enable)
    //   this.canvas.set({ selectable: true, editable: true, selection: false });
    //   this.canvas._objects.map((object) => {
    //     switch (object.type) {
    //       case 'path':
    //         // path cant edit & select
    //         object.set(this.pathObject);
    //         break;
    //       default:
    //         object.set({ selectable: true, selection: true, editable: true, hoverCursor: this.fabricCursor.object });
    //         break;
    //     }
    //   });
    // } else {
    //   // remote user
    //   this.offAll();
    //   // share pointer mode off
    //   this.cursor = 'hide';
    //   this.sendMessage({
    //     event: `pointer_${this.cursor}`,
    //     clientKey: this.channel.clientKey,
    //   });
    //   // canvas & objects change options (select disable)
    //   // this.canvas.set(this.pathObject);
    //   // this.canvas._objects.map((object) => {
    //   //   object.set(this.pathObject);
    //   // });
    // }
